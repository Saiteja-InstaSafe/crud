    ...
    document.getElementById("add-partner-button").addEventListener("click", function() {
        ...
        if (primaryAdminEmail && primaryAdminName && partnerName) {
            ...
            console.log(payl);
                       ...
                       fetch("http://localhost:3030/addTenants",{
                           method:'POST',
                           headers: {
...
$(document).on('click','button.btn.btn-danger.btn-link.btn-sm',function () {
    ...
    if (result) {
        var id = $(this).closest('tr').children('td:first').text();tes
        var payl = {
            id
        };
        fetch("http://localhost:3030/deleteTenant",{
            method:'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
...
function fetchtenants(id) {
    fetch(`http://localhost:3030/getTenants/${id}`)
        .then((data) => data.json())
        .then((data) => {
          //  parterData = [];
